...
var filtertests = [{
	...
	state: {layers: {somelayer: {a:[{foo:"blaj"},{foo:"bar"}],b:[{baz:"bin"}],c:[{foo:"bar"}]}, someotherlayer:{a:[{what:"ev"}],b:[{wuu:"meee"}]}}},
	expected: {a:[{foo:"bar"}]}
}];
	...
	_.each(filtertests,function(test){
		describe("when called with "+JSON.stringify(test.filter)+" and state is "+JSON.stringify(test.state),function(){
			it("returns "+JSON.stringify(test.expected),function(){
				var result = Algol.generateFilter(I.fromJS(test.state||{}),test.filter);
